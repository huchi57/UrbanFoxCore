...
namespace UrbanFox.Editor
{
    ...
    public class GameObjectPainterEditor : UnityEditor.Editor
    {
            ...
            if (Physics.Raycast(mouseRay, out var hit, float.MaxValue, m_layerMask.intValue))
            {
                ...
                switch (m_gameObjectPainter.CurrentBrushShape)
                {
                    case GameObjectPainter.BrushShape.Sphere:
                        DrawZTestWireDisc(m_mouseRayHitPoint, Vector3.up, m_brushRadius.floatValue, defaultColor, occludeColor);
                        DrawZTestWireDisc(m_mouseRayHitPoint, Vector3.right, m_brushRadius.floatValue, defaultColor, occludeColor);
                        DrawZTestWireDisc(m_mouseRayHitPoint, Vector3.forward, m_brushRadius.floatValue, defaultColor, occludeColor);
                        break;
                    case GameObjectPainter.BrushShape.Cylinder:
                        DrawZTestWireDisc(m_mouseRayHitPoint, m_mouseRayHitNormal, m_brushRadius.floatValue, defaultColor, occludeColor);
                        DrawZTestWireDisc(m_mouseRayHitPoint + m_brushHeight.floatValue * m_mouseRayHitNormal, m_mouseRayHitNormal, m_brushRadius.floatValue, defaultColor, occludeColor);
                        DrawZTestWireDisc(m_mouseRayHitPoint - m_brushHeight.floatValue * m_mouseRayHitNormal, m_mouseRayHitNormal, m_brushRadius.floatValue, defaultColor, occludeColor);
                        break;
                    case GameObjectPainter.BrushShape.Box:
                        ...
                        DrawZTestWireCube(m_mouseRayHitPoint, boxRotation, boxSize, defaultColor, occludeColor);
                        break;
                    default:
                        break;
                }
                ...
                if (testInstance)
                {
                    switch (m_gameObjectPainter.CurrentBrushShape)
                    {
                        case GameObjectPainter.BrushShape.Sphere:
                            ...
                            {
                                HandlesExtensions.DrawWireDisc(testInstance.transform.position, Vector3.up, 1, Color.red);
                        ...
                        case GameObjectPainter.BrushShape.Box:
                            ...
                            {
                                DrawZTestWireDisc(testInstance.transform.position, Vector3.up, 1, Color.red, Color.red.SetAlpha(0.25f));
                                m_gameObjectsToBeDeleted.Add(testInstance.gameObject);
                            }
                            break;
                        default:
        ...
        #region Z-Test Draw Function
        private void DrawZTestWireDisc(Vector3 point, Vector3 normal, float radius, Color defaultColor, Color occludeColor)
        {
            using (new HandlesScope(defaultColor, CompareFunction.LessEqual))
            {
                HandlesExtensions.DrawWireDisc(point, normal, radius, defaultColor);
            }
            using (new HandlesScope(occludeColor, CompareFunction.Greater))
            {
                HandlesExtensions.DrawWireDisc(point, normal, radius, occludeColor);
            }
        }
        private void DrawZTestWireCube(Vector3 point, Quaternion rotation, Vector3 scale, Color defaultColor, Color occludeColor)
        {
            using (new HandlesScope(defaultColor, CompareFunction.LessEqual))
            {
                HandlesExtensions.DrawWireCube(point, rotation, scale, defaultColor);
            }
            using (new HandlesScope(occludeColor, CompareFunction.Greater))
            {
                HandlesExtensions.DrawWireCube(point, rotation, scale, occludeColor);
            }
        }
